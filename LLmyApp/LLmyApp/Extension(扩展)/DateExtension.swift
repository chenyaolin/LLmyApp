...
//
//
//  Created by chenyaolin on 16/7/11.
...
extension String {
    ...
    /** 时间戳转时间String */
    static func timeStamp(FormatterStr formatterStr: String, timeStamp: NSInteger) -> String {
...
extension Int {
    ...
    func weekStr(WithSimple isSimple: Bool) -> String {
        var week = ""
        switch self {
        case 1:
...
extension Date {
    ...
    init(year: Int, month: Int = 1, weekday: Int = 1, day: Int = 1, hour: Int = 0, minute: Int = 0, second: Int = 0) {
        let now = Date()
        var dateComponents = Calendar.current.dateComponents([.era, .year, .month, .weekday, .day, .hour, .minute, .second, .nanosecond], from: now)
    ...
    static func dateTimeDifference(WithStartTime startTime: Date, endTime: Date) -> DateComponents {
        let calendar = NSCalendar.current
        let unitFlags: Set<Calendar.Component> = [.year, .month, .day, .hour, .minute, .second]
        let components = calendar.dateComponents(unitFlags, from: startTime, to: endTime)
        return components
    }
